...
def test_run_once_returns_aucs_for_all_detectors():
    ...
    assert set(out) == {"dca", "naive_fusion", "raw_gap", "isolation_forest", "slice_finder"}
    assert all(0.0 <= v <= 1.0 for v in out.values())


def test_run_once_weak_regime_runs():
    out = run_once(_toy_dataset(), "ball", seed=1, weak=True, max_n=400, k=10)
    assert all(0.0 <= v <= 1.0 for v in out.values())
